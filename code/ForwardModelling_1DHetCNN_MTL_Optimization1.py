...
# 定义搜索区间（归一化后的几何参数范围）
pbounds = {f'param_{i}': (0.0, 1.0) for i in range(8)}
...
print("Starting Bayesian Optimization for design...")
start_time = time.time()
optimizer_bo.maximize(init_points=10, n_iter=90)
end_time = time.time()
elapsed_time = end_time - start_time
print("Optimization time: {:.6f} seconds".format(elapsed_time))
print("Best design found (normalized):")
print(optimizer_bo.max)


# 将归一化的几何参数反归一化为实际几何参数
best_norm_params = optimizer_bo.max['params']
best_norm_vector = np.array([best_norm_params[f'param_{i}'] for i in range(8)], dtype=np.float32)
# 反归一化公式： actual = norm * (max - min) + min
...
# 绘制寻优过程的曲线
iterations = np.arange(1, len(optimizer_bo.space.target) + 1)
values = np.array(optimizer_bo.space.target)
...
df_curve.to_csv('ForwardModelling/HetCNN_1D_MLT/bo_curve.csv', index=False)
print("Best value curve saved to 'bo_curve.csv'")"""